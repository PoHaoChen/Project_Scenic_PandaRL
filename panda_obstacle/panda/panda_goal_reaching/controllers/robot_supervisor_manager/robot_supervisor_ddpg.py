...
class PandaRobotSupervisor(RobotSupervisorEnv):
    ...
    def __init__(self):
        ...
        self.obstacle = self.getFromDef("OBSTACLE")
    ...
    def get_reward(self, action):
        ...
        obstaclePosition = ToArmCoord.convert(obstaclePosition)

        end_effector_position = self.end_effector.getPosition()
        end_effector_position = ToArmCoord.convert(end_effector_position)
        joint1_position = ToArmCoord.convert(self.getFromDef("link1").getPosition())
        joint2_position = ToArmCoord.convert(self.getFromDef("link2").getPosition())
        joint3_position = ToArmCoord.convert(self.getFromDef("link3").getPosition())
        joint4_position = ToArmCoord.convert(self.getFromDef("link4").getPosition())
        joint5_position = ToArmCoord.convert(self.getFromDef("link5").getPosition())
        joint6_position = ToArmCoord.convert(self.getFromDef("link6").getPosition())
        joint7_position = ToArmCoord.convert(self.getFromDef("link7").getPosition())
        ...
        obstacleDistance = np.linalg.norm([obstaclePosition[0]-end_effector_position[0],obstaclePosition[1]-end_effector_position[1],obstaclePosition[2]-end_effector_position[2]])
        joint1_Distance = np.linalg.norm([obstaclePosition[0]-joint1_position[0],obstaclePosition[1]-joint1_position[1],obstaclePosition[2]-joint1_position[2]])
        joint2_Distance = np.linalg.norm([obstaclePosition[0]-joint2_position[0],obstaclePosition[1]-joint2_position[1],obstaclePosition[2]-joint2_position[2]])
        joint3_Distance = np.linalg.norm([obstaclePosition[0]-joint3_position[0],obstaclePosition[1]-joint3_position[1],obstaclePosition[2]-joint3_position[2]])
        joint4_Distance = np.linalg.norm([obstaclePosition[0]-joint4_position[0],obstaclePosition[1]-joint4_position[1],obstaclePosition[2]-joint4_position[2]])
        joint5_Distance = np.linalg.norm([obstaclePosition[0]-joint5_position[0],obstaclePosition[1]-joint5_position[1],obstaclePosition[2]-joint5_position[2]])
        joint6_Distance = np.linalg.norm([obstaclePosition[0]-joint6_position[0],obstaclePosition[1]-joint6_position[1],obstaclePosition[2]-joint6_position[2]])
        joint7_Distance = np.linalg.norm([obstaclePosition[0]-joint7_position[0],obstaclePosition[1]-joint7_position[1],obstaclePosition[2]-joint7_position[2]])
        joint_Distance_list = [obstacleDistance, joint1_Distance, joint2_Distance, joint3_Distance, joint4_Distance, joint5_Distance, joint6_Distance, joint7_Distance]
        ...
        if self.distance < 0.01:
            ...
            reward = reward + 0.5
        for jointDistance in joint_Distance_list:
            if (jointDistance > 0.15):
                jointDistance = 0.15
            reward = reward + jointDistance / 7.0
            if jointDistance < 0.01:
                reward = reward - 1.5
                self.collision = True
            elif jointDistance < 0.015:
                reward = reward - 1.0
                self.collision = True
            elif jointDistance < 0.03:
                reward = reward - 0.5
                self.collision = True
            elif jointDistance < 0.005:
                reward = reward - 30.0
                self.collision = True
        return reward
